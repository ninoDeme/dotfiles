...
local btntooltip = awful.tooltip { }
...
btntooltip:add_to_object(suspend)
suspend:connect_signal(
  'mouse::enter', function()
    btntooltip.text = "Suspend"
  end
)
...
btntooltip:add_to_object(lock)
lock:connect_signal(
  'mouse::enter', function()
    btntooltip.text = "Lock"
  end
)
...
local goodbye = wibox.widget.textbox("goodbye")
  ...
  nil,
  {
    nil,
    {
      nil,
        {
          {
            markup = '<span color="'.. beautiful.fg_focus ..'">Goodbye</span>',
            align = 'center',
            widget = wibox.widget.textbox,
            font = "NotoSans Nerd Font 50"
          },
          bottom = dpi(50),
